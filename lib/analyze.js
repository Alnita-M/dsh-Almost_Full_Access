...
export function analyzeCommand(command, workspace, workdir) {
  ...
  // v1.2.0：单一规则集 —— 确定性只拦"不可逆（删除/清空/卸载/格式化）或影响系统正确运行
  // （系统级命令/写系统关键路径）"；可逆写（新建/复制/解压到普通路径）与用户级注册表直接放行；
  // 动态/间接命令由 LLM 子代理审查兜底（见函数末尾 ambiguous）。
  ...
  const pushTarget = (raw) => {
    ...
    if (low.startsWith('hklm') || low.startsWith('hkcu') || low.startsWith('hkcr') || low.startsWith('hku') || low.startsWith('hkcc')) {
      // HKCU/HKCR/HKCC 用户级注册表可逆，放行；HKLM 系统注册表仍拦
      if (low.startsWith('hkcu') || low.startsWith('hkcr') || low.startsWith('hkcc')) return
      if (targets.indexOf('reg:' + t) < 0) targets.push('reg:' + t)
      return
    }
    if (!lenientPathRelevant(t)) return
  ...
  // v1.2.0：安装类（software/packages 安装）与用户级环境变量（setx 非 /M）可逆，放行；
  ...
  // v1.2.0：动态/间接命令交 LLM 子代理审查（判 risky 才弹面板，safe 直接放行）；
  // 字符串拼接与 $(…) 在写意图命令内同样送审（防拼接构造写动词），纯插值不送。
  let ambiguous = AMBIGUOUS_RULES.some((re) => re.test(command))
  if (ambiguous === false && hasWriteIntent) {
