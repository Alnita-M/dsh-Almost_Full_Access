...
export const UNINSTALL_RULES = [
  { id: 'sys:uninstall', re: /\bmsiexec\s+\/(x|a)\b|\b(winget|choco|scoop)\s+uninstall\b|\b(pip|pip3|pipx)\s+uninstall\b|\bconda\s+remove\b|\bgem\s+uninstall\b/i, text: '卸载/移除已安装软件或包（不可逆）' }
]
// v1.1.0：宽松模式删除类写动词（不可逆：目标工作区外即拦）
const DELETE_VERB_RE = /\b(Remove-Item|Remove-ChildItem|rm|del|erase|rd|rmdir|Clear-Content|Clear-Item|Remove-ItemProperty)\b/i
...
export function analyzeCommand(command, workspace, workdir, mode) {
  ...
  const ambiguous = lenient ? false : AMBIGUOUS_RULES.some((re) => re.test(command))
