...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		async function decide(approvalId, decision, rememberCategory) {
			...
			try {
				...
				if (rememberCategory !== undefined && rememberCategory !== null && rememberCategory !== "") payload.rememberCategory = rememberCategory;
		...
		function ApprovalRoot() {
			...
			const rememberKey = rec.category || "";
			const rememberPayload = store.remember ? rememberKey : undefined;
			...
			return React.createElement("div", { className: "afacc-float" },
				...
				React.createElement("section", {
					...
					React.createElement(PanelHeader, { record: rec, onClose: () => { if (decided === null) decide(rec.approvalId, "denied", undefined); } }),
					React.createElement("div", { className: "afacc-body" },
						...
						decided === null ? React.createElement(RememberCheckbox, { checked: store.remember, onChange: (v) => { store.remember = v; emit(); } }) : null
					),
		...
		if (typeof document !== "undefined") {
			document.addEventListener("keydown", (e) => {
				if (e.key === "Escape" && store.items.length > 0) {
					...
					if (head) decide(head.approvalId, "denied", undefined);
				}
			});
		}
