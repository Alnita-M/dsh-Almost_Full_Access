...
const REVIEW_PROMPT = [
  ...
  '1. 影响工作区以外的文件：凡是命令会对工作区目录之外的任何路径产生 修改/删除/创建/移动/解压写入 效果（包括 Windows 路径、%TEMP%、$env:、UNC、/etc /usr /tmp 等类 Unix 绝对路径、注册表导入、解压工具 -C/-DestinationPath 目标、下载落地 -o/OutFile 目标、certutil/bitsadmin 落地文件），一律算此风险。仅读取或查询（如 Get-Process、Get-Service、读取系统信息、查询注册表、Get-Content 读取文件、tar -t 列表）不算。',
  '2. 影响电脑正常开机：修改引导配置（bcdedit/BCD、EFI、引导加载器、MBR、启动项）、磁盘分区操作（diskpart、分区表、格式化）、关机或重启（如 shutdown /r）、禁用休眠等。',
  '3. 影响操作系统与驱动的正常运作：停止/禁用系统服务（Stop-Service、Set-Service、sc config、net stop）、安装/卸载/更新驱动（pnputil、fltmc）、替换或删除系统文件（takeown、icacls、sfc、Remove-Item 系统路径）、修改组策略、写入 HKLM 注册表（reg add/import HKLM、Set-ItemProperty HKLM）、修改用户账户或组成员（net user、net localgroup）、删除卷影副本/还原点（vssadmin delete）、持久化环境变量（setx）、关闭防火墙/杀毒、执行系统更新（DISM、Windows Update）等。仅查询不算。',
  '',
...
function apply(ctx) {
  ...
  // v1.1.0：Almost Full Access 为单一权限预设；其下两个审查分支由输入框旁的选择器切换：
  // fast（🛡️ Fast Mode 宽松：只拦不可逆/系统级）与 safe（🛡️ Safe Mode 严肃：全量规则，默认）
  const GATED_PRESETS = new Set(['almost-full-access'])
  const modeBySession = new Map()   // sessionId -> 'fast' | 'safe'
  const presetBySession = new Map() // sessionId -> 最近一次门控看到的预设名
  // v1.1.1：Safe 模式 LLM 审查结果缓存（同命令 5 分钟复用，避免重复起子代理造成繁琐）
  const llmVerdictCache = new Map() // commandHash -> { verdict, impacts, ts }
  const LLM_CACHE_TTL = 300000
  ...
  // ============ 分支选择（Fast/Safe）数据通道 ============
  // GET  /api/afaccess/mode?session=<id> —— 返回该会话当前分支与预设（供输入框选择器初始化）
  // POST /api/afaccess/mode {session, mode:'fast'|'safe'} —— 切换分支（CSRF 防护同 decide）
  ctx.effect(() => webServer.register({
    kind: 'exact',
    path: '/api/afaccess/mode',
    handler: async (req, res) => {
      if (req.method === 'GET') {
        const session = String(req.url.split('?')[1] || '').replace(/^session=/, '') || ''
        const mode = modeBySession.get(session) || 'safe'
        json(res, 200, { ok: true, mode, preset: presetBySession.get(session) || 'unknown' })
        return
      }
      if (req.method !== 'POST') return json(res, 405, { ok: false, reason: 'method' })
      if (req.headers['x-afaccess-client'] !== '1') return json(res, 403, { ok: false, reason: 'origin' })
      ...
      const args = await readBody(req)
      if (args === null || typeof args !== 'object') return json(res, 400, { ok: false, reason: 'body' })
      const session = typeof args.session === 'string' && args.session !== '' ? args.session : ''
      const mode = args.mode === 'fast' ? 'fast' : args.mode === 'safe' ? 'safe' : ''
      if (session === '' || mode === '') return json(res, 400, { ok: false, reason: 'params' })
      modeBySession.set(session, mode)
      audit('MODE session=' + session + ' -> ' + mode)
  ...
  const diagTool = defineTool({
    ...
    execute: async () => ({
      ...
      modes: Array.from(modeBySession.entries()).map(([s, m]) => s + ' -> ' + m)
    })
  })
  ...
  ctx.on('tools/pre-execute', async (exec, next) => {
    ...
    if (!GATED_PRESETS.has(preset)) return next()
    // v1.1.0：记录该会话预设（供输入框分支选择器判断可见性），并读取分支（默认 safe=严肃）
    const sessionId = agent.session !== undefined ? agent.session.id : ''
    if (sessionId !== '') presetBySession.set(sessionId, preset)
    const mode = modeBySession.get(sessionId) || 'safe'
    const analyzeMode = mode === 'fast' ? 'lenient' : 'strict'
    ...
    audit('GATE ' + exec.name + ' preset=' + preset + ' branch=' + mode + ' cmd=' + command.slice(0, 200).replace(/\n/g, ' '))

    const analysis = analyzeCommand(command, workspace, workdir, analyzeMode)
    audit('ANALYZE mode=' + analyzeMode + ' ambiguous=' + String(analysis.ambiguous) + ' impacts=' + JSON.stringify(analysis.impacts))
    ...
    if (analysis.impacts.length > 0) {
      ...
      // v1.1.1：同命令 LLM 审查结果 5 分钟内复用（Safe 模式去繁琐；命令文本不变则结论不变）
      const cmdHash = hashCommand(command)
      const cached = llmVerdictCache.get(cmdHash)
      if (cached !== undefined && Date.now() - cached.ts < LLM_CACHE_TTL) {
        review = { verdict: cached.verdict, impacts: cached.impacts, source: 'llm' }
        audit('LLM-CACHE hit ' + cmdHash + ' -> ' + cached.verdict)
      } else {
        try {
        ...
        if (review.source === 'llm') {
          ...
          llmVerdictCache.set(cmdHash, { verdict: review.verdict, impacts: review.impacts, ts: Date.now() })
        }
      }
    } else {
