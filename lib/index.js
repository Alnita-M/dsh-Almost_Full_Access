...
function apply(ctx) {
  ...
  // v1.1.0：两个审查模式 —— lenient（🛡️ Fast Mode 宽松：只拦不可逆/系统级）与 strict（🛡️ Safe Mode 严肃：全量规则）
  const GATED_PRESETS = new Set(['lenient', 'strict'])
  ...
  if (permissionPresets !== undefined) {
    try {
      permissionPresets.optionOf('lenient')
      permissionPresets.optionOf('strict')
      state.presetAvailable = true
    } catch (err) {
      ctx.logger.warn('afaccess: presets lenient/strict not advertised: ' + String(err?.message ?? err))
    }
  }
  ...
  ctx.on('tools/pre-execute', async (exec, next) => {
    ...
    const analysis = analyzeCommand(command, workspace, workdir, preset)
    audit('ANALYZE mode=' + preset + ' ambiguous=' + String(analysis.ambiguous) + ' impacts=' + JSON.stringify(analysis.impacts))
